...
- **Design rule: in Outlook dark, dark text and light grounds never
  ...
  this at build and the importer's Export panel warns in its template tab, with
...
| `data-border-toggle="#hex"` | on an `mj-text` that is its column's ONLY member and whose own padding is 0: opt it IN to a Box Border (None / 2px / 4px, 16px inner padding) plus a nested Box Border Color from the palette's border group, defaulting to the hex (see conventions "Box Border"). Border-safe only in a plain full-width column: never a column with an explicit width, inside an mj-group, or in a background-image section, where exact widths must add up |
...
| `data-no-padding-<side>` | on the frame that authors the `padding` shorthand (`top`/`right`/`bottom`/`left`): no Select for exactly that side — it stays a literal px value; the other three sides are untouched. The per-side removal for a control proven dead at BOTH viewports: a px column group pins the gutter at desktop while a flush-mobile rule zeroes it below the breakpoint, and no alternate arrangement can unpin it (2026-08-25, Podcast Streaming Block and Signature Card (photo) row 2). Not for a side that is dead at only ONE viewport — that is a `data-<viewport>-only-padding-<side>` label, not a removal |
