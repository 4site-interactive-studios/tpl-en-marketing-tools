...
function normalize(body) {
  ...
  let s = stripAnnotationComments(body).replace(
    /\s*data-(style-[a-z-]+|fully-exclude|no-display-toggle|no-link-toggle|no-link-color|no-width-toggle|no-background-color|no-direction-toggle|desktop-only-[a-z-]+|mobile-only-[a-z-]+|inset-toggle|text-size-toggle|visible-duplicate|arrangement-label(?:="[^"]*")?|link-group(?:="[^"]*")?|width-options(?:="[^"]*")?)/g,
    '',
  );
